...
const rekakuInfoContainer = document.getElementById('rekaku-info-container');
const rekakuPrompt = document.getElementById('rekaku-prompt');
const rekakuSend = document.getElementById('rekaku-send');
const rekakuResponseContainer = document.getElementById('rekaku-response-container');
...
function openModal(partId, x, y) {
    ...
    if (partId === 'sign_features') {
        ...
        rekakuInfoContainer.classList.remove('hidden');
        rekakuResponseContainer.innerHTML = '<p class="text-gray-500">I am ready to answer your questions!</p>';
        rekakuPrompt.value = '';
    } else {
        // Show static UI
        staticInfoContainer.classList.remove('hidden');
        rekakuInfoContainer.classList.add('hidden');
...
function closeModal() {
    ...
    setTimeout(() => {
        ...
        // Clear Rekaku state
        rekakuPrompt.value = '';
        rekakuResponseContainer.innerHTML = '';
...
rekakuSend.addEventListener('click', handleRekakuRequest);
rekakuPrompt.addEventListener('keyup', (event) => {
    if (event.key === 'Enter') {
        handleRekakuRequest();
    }
});

async function handleRekakuRequest() {
    const userQuery = rekakuPrompt.value;
    if (!userQuery.trim()) {
        rekakuResponseContainer.innerHTML = '<p class="text-red-500">Please enter a question.</p>';
        return;
    }

    rekakuResponseContainer.innerHTML = '<div class="loader"></div>';

    try {
        const responseText = await callRekakuAPI(userQuery);
        rekakuResponseContainer.innerHTML = `<p>${responseText}</p>`;
    } catch (error) {
        console.error("Rekaku API Error:", error);
        rekakuResponseContainer.innerHTML = '<p class="text-red-500">Sorry, I encountered an error. Please try again.</p>';
    }
}
...
async function callRekakuAPI(userQuery) {
